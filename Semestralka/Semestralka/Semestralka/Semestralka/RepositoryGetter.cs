...
namespace RepositoryModel
{
    public class RepositoryGetter
    {
        ...
        {
            if (Semestralka.Connection.CheckConnection())
            {
                try
                {
                ...
                {
                    MessageBox.Show("An error is occured");
                }
            }
            return null;
        }
         ...
         */
        public async Task<IDictionary<string, List<string[]>>> ChangedFiles(DateTime date, CancellationToken token)
        {
            if (Semestralka.Connection.CheckConnection())
            {

                 var commits = await repositoryCommits().ConfigureAwait(false);
                 ...
                 foreach (var currentCommit in commits)
                 {
                    ...
                    if (!Semestralka.Connection.CheckConnection())
                    {
                         MessageBox.Show("changed files exact commit");
                    }
                    if (token.IsCancellationRequested)
                        token.ThrowIfCancellationRequested();

                    // Commit detail
                    var commit = await client.Repository.Commit.Get(UserName, RepoName, currentCommit.Sha).ConfigureAwait(false);

                    // Commit after date (method parameter)
                    if (DateTimeOffset.Compare(commit.Commit.Author.Date, new DateTimeOffset(date)) > 0)
                    {
                        foreach (GitHubCommitFile file in commit.Files)
                        {
                            ...
                            if (isFileBinary(fileData, commit).Result)
                            {
                                version[1] = "binary";
                            }
                            else
                            ...
                            version[2] = commit.Commit.Author.Date.ToLocalTime().ToString("dd.MM.yyyy HH:mm:ss");
